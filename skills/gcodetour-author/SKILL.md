...
**C4 diagrams** ignore the PlantUML `!theme` gallery above (C4 paints its own element colors), but C4-PlantUML has its **own** theme gallery — apply one with the `from` form:

```
!theme C4_united from <C4/themes>
```

The `<C4/themes>` calculated path resolves from the renderer's bundled C4 stdlib, so it works offline with no remote fetch (pinned by the Kroki image digest). Verified working in the pinned renderer: `C4_blue` (the default look), `C4_brown`, `C4_green`, `C4_violet`, `C4_sandstone`, `C4_superhero`, `C4_united`, plus the redesigned `C4_blue_new` / `C4_brown_new` / `C4_green_new` / `C4_violet_new` variants (rounded, wireframe styling). For control beyond a theme, use C4's styling API: `UpdateElementStyle()`, element tags via `AddElementTag` + `$tags=`, `UpdateBoundaryStyle()` / `UpdateRelStyle()`. (C4 themes bake an opaque background like the default, so the self-contained-card guidance below still holds.)
