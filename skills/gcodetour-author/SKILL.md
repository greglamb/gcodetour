...
6. **Self-verify, then read it back.** Run the mechanical verifier *and* do the [final read-through](#final-read-through-agent-task--after-the-verifier-is-green) — a green verifier proves anchors/links resolve, not that the tour opens in the right place or reads well (see Verification).
...
### Final read-through (agent task — after the verifier is green)

The verifier proves the tour is *mechanically* sound; it cannot tell you it reads well or opens in the right place. **Prose and narrative are a judgment call, so this last pass is yours, not the script's** — a tour can be 100% green and still start mid-story, wander, or repeat itself. Once the verifier passes, read the whole tour back end to end **as the reader will** (skim the rendered descriptions in order, not the JSON) and fix what only a reader notices:

- **The opening frames the tour.** Step 1 is a content step that says what the tour covers (and, optionally, how to run the project). A welcome/intro/overview step stranded mid-tour is the single most common structural slip — move it to step 1. *(The verifier flags an intro-like heading that isn't first under `informational: step order & placement` — act on it.)*
- **The order tells one story.** Steps advance the narrative arc you chose (entry → layers → recap), not the order files happen to sit on disk. A backwards jump within a file should be deliberate (a caller shown after its callee), never accidental. *(The verifier flags consecutive same-file steps that jump backwards — confirm each is intentional or reorder.)*
- **The tree reads as an outline.** Skim the `### Heading`s top to bottom: they should scan like a table of contents — every step has one, there are no duplicates or orphans, and a recap (if any) is last.
- **Every step earns its place.** Cut steps that only narrate what the line plainly does; merge near-duplicates; confirm each carries an invariant, a decision (and the road not taken), or a coupling the reader couldn't get by re-reading the code.
- **It reads as prose.** For each step, eyeball the *rendered* description for coherence, correctness, and the markdown hazards the verifier can't see (mid-word bold, broken headings/tables) — see "Markdown rendering isn't verified" above.

This pass is what turns "mechanically valid" into "worth playing." Don't skip it because the verifier is green — green is necessary, not sufficient.
