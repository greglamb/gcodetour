...
The `<C4/themes>` calculated path resolves from the renderer's bundled C4 stdlib, so it works offline with no remote fetch (pinned by the Kroki image digest). Verified working in the pinned renderer: `C4_blue` (the default look), `C4_brown`, `C4_green`, `C4_violet`, `C4_sandstone`, `C4_superhero`, `C4_united`. The redesigned `C4_*_new` variants and `!NEW_C4_STYLE=1` are **not** available in the pinned image — don't use them. For control beyond a theme, use C4's styling API: `UpdateElementStyle()`, element tags via `AddElementTag` + `$tags=`, `UpdateBoundaryStyle()` / `UpdateRelStyle()`. (C4 themes bake an opaque background like the default, so the self-contained-card guidance below still holds.)
